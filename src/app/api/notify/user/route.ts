...
export async function POST(req: Request) {
  if (!LOFT_SHOP_TOKEN) {
    return NextResponse.json({ message: "LOFT_SHOP_TOKEN is not configured" }, { status: 500 });
  }

  const { recipientId, messageText } = await req.json();
  if (!recipientId || !messageText) {
    return NextResponse.json({ message: "recipientId and messageText are required" }, { status: 400 });
  }
